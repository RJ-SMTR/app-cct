...
export default function BasicEditingGrid() {
  ...
  const handleLoadMore = () => {
    ...
    console.log("Data being sent for load more:", data);
    onSubmit(data, { customPage: nextPage, customLimit: rowsPerPage, append: true });
  };
  ...
  const onSubmit = (data, options = {}) => {
    const { customPage = page, customLimit = rowsPerPage, append = false } = options;
    ...
    dispatch(handleReportInfo(requestData, reportType))
      .then((response) => {
        ...
        if (response?.data?.length) {
          setAllReports(prev => append ? [...prev, ...response.data] : response.data);

          setHasMore(response.data.length === customLimit);
          console.log("Updated allReports, hasMore:", hasMore);
        } else {
          setHasMore(false);
        }
      })
