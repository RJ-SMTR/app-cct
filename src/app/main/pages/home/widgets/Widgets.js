...
export function TripsResume({ mapInfo, statements }) {
    ...
    useEffect(() => {
        if (mapInfo.length > 0 && mapRef.current) {
            const averageLat = mapInfo.reduce((sum, point) => sum + point.transactionLat, 0) / mapInfo.length;
            const averageLon = mapInfo.reduce((sum, point) => sum + point.transactionLon, 0) / mapInfo.length;
