...
export function buildAgentConsolidatedReportParams(filters = {}) {
  ...
  const hasAllErrorReasons =
    selectedErrorReasons.size === 0 ||
    selectedErrorReasons.has(AGENT_REPORT_SELECT_ALL_VALUE);

  if (selectedStatuses.has("Pago")) {
    params.pago = true;
  }

  if (selectedStatuses.has("A pagar")) {
    params.aPagar = true;
  }
  if (selectedStatuses.has("Em processamento")) {
    params.emProcessamento = true;
  }
  if (hasErrorStatus && (hasAllErrorReasons || selectedErrorReasons.has("Rejeitado"))) {
    params.rejeitado = true;
  }
  if (hasErrorStatus && (hasAllErrorReasons || selectedErrorReasons.has("Estorno"))) {
    params.estorno = true;
  }
